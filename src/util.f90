...
!------------------------------------------------------------------------------
! $Header: /opt/cvs/phsht/WFplot/util.f90,v 1.1 2008/06/13 16:46:37 phsht Exp $
!------------------------------------------------------------------------------
...
SUBROUTINE DRAW_CUBE_POSITIVE(IX,IY,IZ,SIZE,EDGED,POSITIVE,PS_UNIT)
  IMPLICIT NONE
  DOUBLE PRECISION X,Y,Z,SIZE
  INTEGER PS_UNIT,I,IP(8,2),IX,IY,IZ
  LOGICAL EDGED,POSITIVE
  ...
  call ps_line(PS_UNIT,IP(4,1),IP(4,2),IP(7,1),IP(7,2))
  call ps_line(PS_UNIT,IP(6,1),IP(6,2),IP(7,1),IP(7,2))
END SUBROUTINE DRAW_CUBE_POSITIVE
